...
const ExamStarted = (props) => {
    ...
    const countdown = useCountdown("00:27:00")
                ...
                <div className="BodyWrapper">
                    ...
                    <div className={mockCss.QuestionArea}>
                        <h4>
                            <span style={{ color: 'black' }}>
                                {accounting2020[currentQuestion].question_number}
                            </span> of {totalQuestions}
                        </h4>
                        <p className={mockCss.QuestionText}>
                            {accounting2020[currentQuestion].question_text}
                        </p>
                        <div className={mockCss.OptionsList}>
                            {accounting2020[currentQuestion].options.map((opt, idx) => (
                                <label key={idx} className={mockCss.OptionLabel}>
                                    <input
                                        type="radio"
                                        name={`question_${currentQuestion}`}
                                        checked={answers[accounting2020[currentQuestion].question_number] === idx}
                                        onChange={() => handleOptionChange(accounting2020[currentQuestion].question_number, idx)}
                                    />
                                    <span>{opt}</span>
                                </label>
                            ))}
                    ...
                    <div className="Segment">
                        ...
                        <div className={mockCss.QuickNavArea}>
                            {accounting2020.map((q, idx) => (
                                <button
                                    key={q.question_number}
                                    className={`${mockCss.QuickNavBtn} ${currentQuestion === idx ? mockCss.ActiveQuickNav : ""}`}
